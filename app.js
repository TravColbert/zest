...
module.exports = function (explicitConfig = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    !!(app.locals.nodeEnv !== "production") &&
      console.debug(` config: APP_ROOT => ${app.locals.appRoot}`)
    app.locals.webRoot = getConfigValue(
      "WEB_ROOT",
      "site",
      app.locals.nodeEnv !== "production"
    )
    ...
    app.locals.cachingDisabled = getConfigValue(
      "CACHING_DISABLED",
      true,
      app.locals.debug
    )
    app.locals.cachingDisabled && app.disable("etag")
    app.locals.disableXPoweredBy = getConfigValue(
     ...
     * to a handler somewhere else deeper down the tree.
     *
    ...
    app.locals.authenticate = getConfigValue(
      "AUTHENTICATE",
      "local",
      app.locals.debug
    )
    app.locals.appDescription = getConfigValue(
      "APP_DESCRIPTION",
      "Effortlessly build performant web applications using native web practices",
      app.locals.debug
    )
    app.locals.appImage = getConfigValue(
      "APP_IMAGE",
      "/images/zest.png",
      app.locals.debug
    )
    ...
    app.locals.appUrl = getConfigValue(
      "APP_URL",
      "https://localhost:3000",
      app.locals.debug
    )
    app.locals.language = getConfigValue("LANGUAGE", "en", app.locals.debug)
    app.locals.noResponseCodes = getConfigValue(
      "NO_RESPONSE_CODES",
      false,
      app.locals.debug
    )
    ...
    app.locals.authorName = getConfigValue(
      "AUTHOR_NAME",
      "Travis Colbert",
      app.locals.debug
    )
    app.locals.authorLink = getConfigValue(
      "AUTHOR_LINK",
      "https://github.com/TravColbert/zest",
      app.locals.debug
    )
    ...
    return require(path.join(__dirname, "/lib/zest-db"))(
      app.locals.dbConfig,
      app.locals.nodeEnv,
      app.locals.debug
    )
      .then(db => {
        ...
        app.use(
          session({
            ...
            saveUninitialized: getConfigValue(
        ...
        // Require the right authentication module
        let authenticator = false
        if (app.locals.authenticate) {
          let authenticatorPath = path.join(
            __dirname,
            "auth",
            `${app.locals.authenticate}.js`
          )
          app.locals.debug &&
            console.debug(
              `🟡 searching for authenticator at: ${authenticatorPath}`
            )
        ...
        // Set up Zest
        app.use(
          require("./lib/zest")(app.locals.webRoot, {
            notFoundHandler,
            authenticator,
          })
        )
