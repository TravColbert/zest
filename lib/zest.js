...
const parsePathParams = (req, res, modulePath) => {
  ...
  let strippedResponderPath = res.locals.responder.replace(absolutePathToSite, '')
  res.app.locals.debug && console.debug(`responder: \t\t${path.dirname(strippedResponderPath)}`)
...
const initRequestHandler = (req, res, next) => {
  if (res.app.locals.nodeEnv !== "production") {
    res.locals.templateModifier = ('_fragment' in req.query) ? req.query._fragment : 'page'
  } else {
...
module.exports = (webRoot, { notFoundHandler, authenticator }) => {
  const mainHandler = async (req, res, next) => {
    ...
    for (let index = 0; index < searchPathList.length; index++) {
      ...
      if (found) {
        const requestParams = requestPath.split('/').filter(pathPart => (['indefined', null, ''].indexOf(pathPart) == -1))
        console.log(`route params:\t\t${JSON.stringify(requestParams)}`)
        req.app.locals.requestedModel = requestParams.slice(-1)
        console.log(`requested model:\t${req.app.locals.requestedModel}`)
        res.locals.responder = searchPathList[index]
        console.log(`responder:\t\t${res.locals.responder}`)
