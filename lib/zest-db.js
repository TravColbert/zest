...
const hydrateDb = async (dbName, dbConfig, environment, debug) => {
  try {
    ...
    if (!fs.existsSync(modelPath)) {
      debug && console.error(" cannot find path to model definition: %s", modelPath)
    ...
    if (fs.existsSync(modelPath)) {
      debug && console.info(`🌑 '${modelPath}' hydration starting`)
      ...
      for (const modelFileName of modelFileNames) {
        ...
        debug &&
          console.debug(
            " model definition: %s:\t%s",
            modelFileName.name,
            modelFile
          )
        const model = require(modelFile)
        if ("definition" in model) await model.definition(db, DataTypes)
        debug && console.info(`🌒 '${modelFileName.name}' defined`)
      }

      // Model Post-definition tasks
      for (const modelFileName of modelFileNames) {
        const modelFile = path.join(modelPath, modelFileName.name)
        debug &&
          console.debug(
            " model post-definition tasks - %s:\t%s",
            modelFileName.name,
            modelFile
          )
        const model = require(modelFile)
        if ("postDefinition" in model)
          await model.postDefinition(db, DataTypes, Op)
        debug && console.info(`🌓 '${modelFileName.name}' hooks added`)
      }
      ...
      if (!fs.existsSync(seedPath)) {
        debug &&
          console.error(
            " cannot find path to seed file for model: %s",
            seedPath
          )
      ...
      if (fs.existsSync(seedPath)) {
        const seedFiles = fs
          .readdirSync(seedPath, { withFileTypes: true })
          .filter(dirEntry => dirEntry.isFile())
          .filter(file => file.name.endsWith(".js"))
        ...
        for (const seedFile of filterByExtension(seedFiles, ".js")) {
          ...
          require(seed)(db)
          debug && console.info(`🌔 '${seedFile.name}' seeded`)
        }
      }
      debug && console.info(`🌕 '${modelPath}' hydration complete`)
    }

    return db
  } catch (error) {
    console.debug(
      `☄️ Zest found this error as it was starting up the database:\n\t${error}`
    )
  }
}
...
module.exports = async function (dbConfig, environment, debug = true) {
  ...
  for (const dbConfigName in dbConfig) {
    dbSources[dbConfigName] = await hydrateDb(
      dbConfigName,
      dbConfig[dbConfigName],
      environment,
      debug
    )
  }
