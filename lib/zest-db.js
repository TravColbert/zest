...
const hydrateDb = async (dbName, dbConfig, environment, debug) => {
  try {    
    ...
    if (fs.existsSync(seedPath)) {
      const seedFileNames = fs.readdirSync(seedPath, { withFileTypes: true }).filter(dirEntry => dirEntry.isFile())
      for (const seedFileName of seedFileNames) {
        debug && console.debug(` found a seed file: ${seedFileName}`)
        const seedFile = path.join(seedPath, seedFileName.name)
        debug && console.debug(' model seeding - %s:\t%s', seedFileName.name, seedFile)
        require(seedFile)(db, debug)
      }
    }

    return db
  } catch(error) {
    console.debug(`Zest found this error as it was starting up:\n\t\t${error}`)
  }
}
